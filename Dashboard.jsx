...
const Dashboard = ({ data }) => {
    ...
    if (validData.length === 0) {
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', background: THEME.bg, color: THEME.primary }} onDrop={onDrop} onDragOver={onDragOver}>
                <div style={{ background: 'white', padding: '3rem', borderRadius: '20px', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1)', maxWidth: '600px', textAlign: 'center', border: `2px dashed ${THEME.border}` }}>
                    <div style={{ marginBottom: '1.5rem', color: THEME.accent }}><FileText size={48} /></div>
                    <h2 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '1rem', color: THEME.primary }}>학습 데이터 대기 중...</h2>
                    <p style={{ marginBottom: '2rem', color: THEME.secondary, lineHeight: '1.6' }}><b>'data'</b> 폴더에 엑셀/CSV 파일을 넣어주세요.<br />자동으로 감지하여 화면이 갱신됩니다.</p>
                    ...
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                        <button onClick={handleLoadDemo} style={{ background: 'white', color: THEME.secondary, border: `1px solid ${THEME.border}`, padding: '0.75rem 1.5rem', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold' }}>데모 데이터 보기</button>
                        ...
                        <button onClick={() => serverFileInputRef.current.click()} style={{ background: THEME.primary, color: 'white', border: 'none', padding: '0.75rem 1.5rem', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
